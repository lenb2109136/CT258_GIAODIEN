            ...
            <ul>
              {
                (t?.thoiGianKhoiHanh2 ?? []).map(data => (
                  <li key={data.thoiGian}>Khởi hành: {formatDate(data.thoiGian)}</li>
                ))
              }
        ...
        (t?.thoiGianKhoiHanh2 ?? []).map((data, index) => (
            <div className="col-md-6" key={index}>
                <ul className="list-unstyled">
                    <li>{data?.thoiGian ? formatDate(data.thoiGian) : "Không có dữ liệu"}</li>
                </ul>
            </div>
        ))
    ) : (
        <p className="text-muted">Không có thông tin khởi hành.</p>
    )
}
          ...
          <button disabled={!chuachon||(t?.thoiGianKhoiHanh2 ?? []).length < 0} onClick={() => {
