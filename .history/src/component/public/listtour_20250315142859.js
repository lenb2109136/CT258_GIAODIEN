...
export default () => {
  ...
  const handleTourSelect = tourTags => {
    if (tourTags) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch(error => {
        console.error('Lỗi khi lấy tour theo loại:', error);
        setlisttour([]);
        setIsLoading(false);
      });
  }, [loaichon]);
  ...
  return (
    <div className="row">
        ...
        <div>
          ...
          >
            {loai?.map(data => (
              ...
              />
            )) || <div>Đang tải danh sách loại...</div>}
          </BottomNavigation>
        </div>
      </div>
      ...
      <div style={{ width: '100%', marginBottom: '15px' }}>
        ...
        {isLoading ? (
          <div>Đang tải tour gợi ý...</div>
        ) : (
          <Slider ds={favorite} onTourClick={handleTourSelect} />
        )}
      </div>
              ...
              {dsNgay.current.map((brand, index) => (
                ...
                >
                  <input
                    onClick={() => {
                      let mm = kiemtra(
                        filter.current.dsNgay || [],
                        brand.batDau
                      );
                      if (mm === -1) {
                        filter.current.dsNgay = filter.current.dsNgay || [];
            ...
            {listtour?.map((data, index) => {
              if (index <= indx + 5 && index >= indx) {
                return (
                  ...
                  >
                    <div
                      onClick={() => handleTourSelect(data.T_TAGS)}
                      style={{ width: '100%' }} // Đảm bảo bao phủ toàn bộ khu vực
                    >
                      <Cart
                        ten={data.T_TEN}
                        id={data.T_ID}
              ...
              (isLoading ? (
                ...
                <div>Không có tour để hiển thị</div>
              ))}

            {listtour.length === 0 && !isLoading && (
