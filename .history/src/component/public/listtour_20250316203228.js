...
import TourCard from '../carttour'; // Thay Cart bằng TourCard
...
export default () => {
  ...
  // Lấy danh sách tour theo loại tour
  useEffect(() => {
    axios.get('http://localhost:8080/tour/getListTour').then(data => {
              ...
              if (index <= indx + 5 && index >= indx) {
                return (
                  <Link
                    ...
                    onClick={e => {
                      e.preventDefault();
                      handleTourSelect(data); // Cập nhật slider
                      setTimeout(() => {
                  ...
                  >
                    <TourCard
                      ...
                      dem={data.T_SODEM}
                      lt_id={data.LT_ID}
                      onTourSelect={handleTourSelect} // Truyền callback vào TourCard
                    />
                  </Link>
                );
              }
