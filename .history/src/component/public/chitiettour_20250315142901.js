...
const TourInfo = () => {
  ...
  const handleTourClick = tourTags => {
    if (tourTags) {
      sessionStorage.setItem('selectedTourTag', tourTags);
      setIsLoading(true);
      axios
        .get(`http://localhost:8080/tour/getListTourByTag?tag=${tourTags}`)
        .then(response => {
          setfavorate(response.data.data || []);
          setIsLoading(false);
        })
        .catch(error => {
          console.error('Lỗi khi lấy tour theo tag:', error);
          setfavorate([]);
          setIsLoading(false);
        });
    }
  ...
  useEffect(() => {
    setIsLoading(true);
    const storedTag = sessionStorage.getItem('selectedTourTag');
    if (storedTag) {
      axios
        .get(`http://localhost:8080/tour/getListTourByTag?tag=${storedTag}`)
        .then(response => {
          setfavorate(response.data.data || []);
          setIsLoading(false);
        })
  ...
  if (isLoading) return <div>Đang tải thông tin tour...</div>;

  return (
    <>
      <div className="container mt-4">
        <h2 className="fw-bold">{t?.ten || 'Tour không xác định'}</h2>
        ...
        <div className="row mt-4">
          <div className="col-md-6">
            <div className="position-relative">
              <img
                ...
                className="img-fluid"
                src={
                  t?.anh || 'https://via.placeholder.com/600x500?text=No+Image'
                }
                alt=""
              />
          ...
          <div className="col-md-6">
            ...
            <div className="d-flex">
              ...
              />
              <p>
                <strong>Thời gian:</strong> {t?.soNgay || 0} ngày{' '}
                {t?.soDem || 0} đêm
              </p>
            </div>
            <div>
              ...
              <ul>
                ...
                )) || <li>Không có thông tin khởi hành</li>}
              </ul>
            </div>
            <div className="d-flex">
              <img
            ...
            <div className="container">
              <div className="row">
                ...
                )) || <div>Không có ngày khởi hành</div>}
              </div>
            </div>
            <button
              disabled={!chuachon}
              onClick={() => {
                ...
                setchu(false);
                if (t?.tourTags) {
                  sessionStorage.setItem('selectedTourTag', t.tourTags);
                  handleTourClick(t.tourTags); // Cập nhật ngay danh sách gợi ý
                }
              }}
              className="w-100"
              style={{
