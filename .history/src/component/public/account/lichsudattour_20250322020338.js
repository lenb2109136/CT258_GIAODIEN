  ...
  return (
    ...
    >
      ...
      <div>
          ...
          <tbody>
            {dsve?.map((data, index) => (
              <tr key={index}>
                ...
                <td style={{ border: '2px solid #7AB730' }}>
                  {formatDateTime(data.V_NGAYDAT)}
                </td>
                <td style={{ border: '2px solid #7AB730' }}>
                  {formatDateTime(data.T_THOIGIANKHOIHANH)}
                </td>
                <td style={{ border: '2px solid #7AB730' }}>{data.V_GIA}</td>
                <td style={{ backgroundColor: '#7AB730', color: 'white' }}>
                  <button
                    onClick={() => {
                      api.post(`ve/huyve?id=${data.v_id}`).then(data => {
        ...
        {dsve?.length === 0 && (
          <h3
            style={{ textAlign: 'center', marginTop: '30px', color: '#7AB730' }}
          >
