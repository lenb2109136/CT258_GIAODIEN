...
const TourInfo = () => {
  ...
  const calculateSimilarity = (tour1, tour2) => {
    console.log('Tour1:', tour1, 'Tour2:', tour2); // Debug
    // Điều chỉnh tên trường dựa trên dữ liệu thực tế từ API
    const vector1 = [
      tour1.T_SONGAY || tour1.soNgay || 0,
      tour1.T_SODEM || tour1.soDem || 0,
      tour1.gia || 0,
    ];
    const vector2 = [
      tour2.T_SONGAY || tour2.soNgay || 0,
      tour2.T_SODEM || tour2.soDem || 0,
      tour2.gia || 0,
    ];
  ...
  const getRecommendedTours = selectedTour => {
    if (!selectedTour || !allTours || allTours.length === 0) {
      console.log('Không đủ dữ liệu để tính tour gợi ý:', {
        selectedTour,
        allTours,
      });
      return [];
    ...
    const recommendedTours = sortedTours.slice(0, 5).map(item => item.tour);
    console.log('Tour gợi ý:', recommendedTours); // Debug
  ...
  useEffect(() => {
    axios
      ...
      .then(data => {
        console.log('Dữ liệu allTours:', data.data.data); // Debug
        setAllTours(data.data.data);
      })
      .catch(error => console.error('Lỗi khi lấy allTours:', error));
  }, []);
  ...
  useEffect(() => {
    axios
      ...
      .then(data => {
        console.log('Dữ liệu favorite ban đầu:', data.data.data); // Debug
        setfavorate(data.data.data);
      })
      .catch(error => console.error('Lỗi khi lấy favorite:', error));
  }, []);
  ...
  useEffect(() => {
    axios
      ...
      .then(data => {
        console.log('Dữ liệu tour chi tiết:', data.data.data); // Debug
        sett(data.data.data);
      })
      .catch(error => console.error('Lỗi khi lấy tour chi tiết:', error));
  }, [id]);
  ...
  // Tính tour tương đồng dựa trên tour được chọn từ sessionStorage hoặc tour hiện tại
  useEffect(() => {
    const selectedTour =
      JSON.parse(sessionStorage.getItem('selectedTour')) || t;
    if (selectedTour && allTours.length > 0) {
      console.log('Tour được chọn:', selectedTour); // Debug
      const recommendedTours = getRecommendedTours(selectedTour);
      setfavorate(
        recommendedTours.length > 0 ? recommendedTours : allTours.slice(0, 5)
      ); // Fallback nếu không có tour gợi ý
    }
  }, [allTours, t]);
