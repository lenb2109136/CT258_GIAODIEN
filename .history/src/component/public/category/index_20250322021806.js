...
export default function TourTypeAdmin() {
  ...
  const fetchTourTypes = () => {
    api
      .get('/loaitour/getall') // Điều chỉnh endpoint nếu cần
      .then(response => {
  ...
  const handleUpdate = () => {
    api
      .put(`/loaitour/update/${editingId}`, editedTourType)
      .then(() => {
        setTourTypes(
          tourTypes.map(type =>
  ...
  // Handle input change for new tour type
  const handleNewTourTypeChange = (field, value) => {
    setNewTourType(prev => ({ ...prev, [field]: value }));
  };

  // Handle add new tour type
  const handleAdd = () => {
    if (!newTourType.ten || !newTourType.icon) {
      alert('Vui lòng điền đầy đủ thông tin!');
      return;
    }
    api
      .post('/loaitour/add', newTourType)
      .then(response => {
    ...
    <div className="container mt-5">
      ...
      <div className="row mb-4 align-items-center">
        ...
        <div className="col-md-6 text-md-end mt-3 mt-md-0">
          ...
          >
            <i className="fas fa-plus me-2"></i> Thêm loại tour
          </button>
        </div>
      </div>
      ...
      {showAddForm && (
        <div className="card mb-4 shadow-sm">
          <div className="card-body">
            <h5 className="card-title">Thêm loại tour mới</h5>
            <div className="row">
              <div className="col-md-5">
                <input
                  ...
                  onChange={e => handleNewTourTypeChange('ten', e.target.value)}
                />
              </div>
              <div className="col-md-5">
                <input
                  ...
                  onChange={e =>
                    handleNewTourTypeChange('icon', e.target.value)
                  }
                />
              </div>
                    ...
                    <td>
                      {editingId === type.id ? (
                        ...
                        <img
                          ...
                          onError={e => (e.target.src = 'default-icon.png')} // Xử lý khi ảnh lỗi
                        />
                      )}
                    </td>
                    <td>
