...
export default function BookingAdmin() {
  ...
  const filteredBookings = bookings.filter(booking => {
    // Ensure booking and vId are defined
    if (!booking || !booking.vId) {
      console.log('Invalid booking:', booking); // Log invalid bookings
      return false;
    }

    const idString = booking.vId.toString().toLowerCase();
    const customerName = (booking.khachHang?.khTen || '').toLowerCase();
    const tourName = (booking.thoiGianKhoiHanh?.tour?.tTen || '').toLowerCase();
  ...
  const handleDelete = bookingId => {
    if (window.confirm('Bạn có chắc chắn muốn xóa vé này không?')) {
      api
        ...
        .then(() => {
          setBookings(bookings.filter(booking => booking.vId !== bookingId));
            ...
            <tbody>
              {filteredBookings.length > 0 ? (
                filteredBookings.map(booking => (
                  <tr key={booking.vId}>
                    <td>{booking.vId}</td>
                    <td className="fw-medium">{booking.vId}</td>
                    <td>{booking.khachHang?.khTen || 'N/A'}</td>
                    <td>{booking.thoiGianKhoiHanh?.tour?.tTen || 'N/A'}</td>
                    <td>{booking.vGia.toLocaleString()} VND</td>
                    <td>{new Date(booking.vNgayDat).toLocaleDateString()}</td>
                    <td>
                      ...
                      <button
                        ...
                        onClick={() => handleDelete(booking.vId)}
                      >
